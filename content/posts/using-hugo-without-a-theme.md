...
*   To define actual pages, put them in content just as you would if you were using a theme. Here's a minimal example:  
    ...
    title: "Main page"  
    ---  
    <p><b>This</b> is the <i>main</i> page content.</p>
    ```
...
*   Now, you can write a minimal template in `layouts/index.html` and `layouts/_default/single.html`: Put the following in both files to override the "main" block from `baseof.html` with an empty string:  
    `{{ block "main" . }}{{ end}}`
